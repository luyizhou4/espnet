...
#SBATCH -c 6
#SBATCH -o logs/ddp.%j
#SBATCH -x gqxx-01-075,gqxx-01-014,gqxx-01-121,gqxx-01-122,gqxx-01-003,gqxx-01-072,gqxx-01-071,gqxx-01-034
#SBATCH --mem=50G
...
train_json=/mnt/lustre/sjtu/users/yzl23/work_dir/asr/is20_codeswitching/espnet/egs/codeswitching/asr/data/bnf_json_data/lid_bnf_tf-12l-asr-init/mix200/data.json
valid_json=/mnt/lustre/sjtu/users/yzl23/work_dir/asr/is20_codeswitching/espnet/egs/codeswitching/asr/data/bnf_json_data/lid_bnf_tf-12l-asr-init/dev_mix20/data.json

tag=bnf/mix200/mix200_jca0.2_gpu2_lid-bnf
expdir=exp/${tag}
mkdir -p ${expdir}
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --mtlalpha ${mtlalpha} \
        --model-module "espnet.nets.pytorch_backend.bottleneck.e2e_asr_transformer_yzl23_bnf:E2E" \
        --moe-mode true \
        --bnf-dim ${bnf_dim} \
        ${pretrained_jca_model:+--pretrained-jca-model $pretrained_jca_model} \
        --epochs ${epochs} \
...
if [ $rank -eq 0 ]; then
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        (
            # split data
            dev_root=/mnt/lustre/sjtu/users/yzl23/work_dir/asr/is20_codeswitching/espnet/egs/codeswitching/asr/data/bnf_json_data/lid_bnf_tf-12l-asr-init/dev_mix20
    ...
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        (
            # split data
            dev_root=/mnt/lustre/sjtu/users/yzl23/work_dir/asr/is20_codeswitching/espnet/egs/codeswitching/asr/data/bnf_json_data/lid_bnf_tf-12l-asr-init/eval_mix20
            splitjson.py --parts ${nj} ${dev_root}/data.json
