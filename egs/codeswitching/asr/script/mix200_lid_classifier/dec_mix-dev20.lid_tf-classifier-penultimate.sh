...
#SBATCH -c 1
#SBATCH -o logs/%j
#SBATCH -x gqxx-01-075,gqxx-01-014,gqxx-01-121,gqxx-01-122,gqxx-01-076
#SBATCH --mem=50G
...
train_config=conf/train_lid_classifier.yaml
...
dec_tag="greedy_decoding_mix20-dev_penultimate"
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    (
        ...
        # split data
        dev_root=/mnt/lustre/sjtu/users/yzl23/work_dir/asr/is20_codeswitching/espnet/egs/codeswitching/asr/data/lang_chunk/hybrid_align_json/dev_mix20_decoding
        splitjson.py --parts ${nj} ${dev_root}/data.json

        #### use CPU for decoding
        ngpu=0

        slurm.pl -x gqxx-01-006 JOB=1:${nj} ${expdir}/${decode_dir}/log/decode.JOB.log \
            asr_recog.py \
            ...
            --store-penultimate-state "ark:| copy-feats ark:- ark,scp:${expdir}/${decode_dir}/moe_coe.JOB.ark,${expdir}/${decode_dir}/moe_coe.JOB.scp" \
    ...
    cat ${expdir}/decode_${dec_tag}/moe_coe.*.scp | sort > ${expdir}/decode_${dec_tag}/moe_coe.scp
    feat-to-len scp:${expdir}/decode_${dec_tag}/moe_coe.scp ark,t:${expdir}/decode_${dec_tag}/moe_coe.len
    echo "Finished"
fi
