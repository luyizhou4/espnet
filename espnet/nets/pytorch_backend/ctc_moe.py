...
class CTC(torch.nn.Module):
    ...
    def __init__(self, odim, eprojs, dropout_rate, ctc_type='warpctc', reduce=True):
        ...
        self.loss = None
        # here we have seperate ctc_lo
        self.cn_ctc_lo = torch.nn.Linear(eprojs, odim)
        self.en_ctc_lo = torch.nn.Linear(eprojs, odim)
    ...
    def forward(self, hs_pad, hlens, ys_pad, moe_coes):
        """CTC forward
        ...
        # zero padding for hs
        hs_pad = F.dropout(hs_pad, p=self.dropout_rate)
        cn_ys_hat = self.cn_ctc_lo(hs_pad)
        en_ys_hat = self.en_ctc_lo(hs_pad)
        ys_hat = cn_ys_hat * moe_coes[:, :, 1] + en_ys_hat * moe_coes[:, :, 0]
        # zero padding for ys
    ...
    def log_softmax(self, hs_pad, moe_coes):
    ...
    def argmax(self, hs_pad, moe_coes):
