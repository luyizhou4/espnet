...
from espnet.nets.pytorch_backend.transformer.label_smoothing_loss import LabelSmoothingLoss, MoELanguageIDMultitakLoss
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        ...
        group.add_argument('--lid-mtl-lsm-weight', default=0.0, type=float,
                           help='Language id multitask label smoothing loss')
        return parser
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        # gated add module 
        self.aggregation_module = torch.nn.Linear(2*args.adim, 2)
        ...
        self.lid_criterion = MoELanguageIDMultitakLoss(
                                size=2, 
                                padding_idx=self.ignore_id, 
                                smoothing=args.lid_mtl_lsm_weight,
    ...
    def forward(self, xs_pad, ilens, ys_pad, moe_coes, moe_coe_lens):
        ...
        lambda_ = F.softmax(
            self.aggre_scaling * self.aggregation_module(hs_pad), -1
            ).unsqueeze(-1)
        ctc_hs_pad = lambda_[:,:,0] * cn_hs_pad + lambda_[:,:,1] * en_hs_pad
        ...
        lid_acc = th_accuracy(lambda_.view(-1, 2), moe_coes,
        ...
        if self.mtlalpha == 0.0:
            ...
            loss_ctc = self.ctc(ctc_hs_pad.view(batch_size, -1, self.adim), hs_len, ys_pad)
            if self.error_calculator is not None:
                ys_hat = self.ctc.argmax(ctc_hs_pad.view(batch_size, -1, self.adim)).data
        ...
        if self.mtlalpha == 1:
            ...
            pred_pad, pred_mask = self.decoder(ys_in_pad, ys_mask, s2s_hs_pad, s2s_hs_mask)
    ...
    def encode(self, x):
        ...
        """
        self.eval()
        x = x[0] # x: fbank_feats, moe_coe, for decoding, we only use fbank_feats
        ...
        lambda_ = F.softmax(
            self.aggre_scaling * self.aggregation_module(enc_output), -1
            ).unsqueeze(-1) 
        ctc_enc_output = lambda_[:,:,0] * cn_enc_output + lambda_[:,:,1] * en_enc_output
        s2s_enc_output = torch.cat((cn_enc_output, en_enc_output), dim=1)
        ...
        return ctc_enc_output, s2s_enc_output # returns tensor(1, T, D)
    ...
    def recognize_ctc_greedy(self, x, recog_args):
        ...
        :rtype: list
        """
        enc_output, _ = self.encode(x) # (1, T, D)
    ...
    def recognize_jca(self, x, recog_args, char_list=None, rnnlm=None, use_jit=False):
        ...
        :rtype: list
        """
        ctc_enc_output, s2s_enc_output = self.encode(x) # (1, T, D)
        if recog_args.ctc_weight > 0.0:
            lpz = self.ctc.log_softmax(ctc_enc_output)
            lpz = lpz.squeeze(0) # shape of (T, D)
        else:
            lpz = None

        h = ctc_enc_output.squeeze(0) # (B, T, D), #B=1
                ...
                if use_jit:
                    if traced_decoder is None:
                        traced_decoder = torch.jit.trace(self.decoder.forward_one_step,
                                                         (ys, ys_mask, s2s_enc_output))
                    local_att_scores = traced_decoder(ys, ys_mask, s2s_enc_output)[0]
                else:
                    local_att_scores = self.decoder.forward_one_step(ys, ys_mask, s2s_enc_output)[0]
                if rnnlm:
