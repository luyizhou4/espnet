...
class Decoder(ScorerInterface, torch.nn.Module):
    ...
    def forward(self, tgt, tgt_mask, cn_memory, en_memory, 
                memory_mask, return_penultimate_state=False):
        """Forward decoder.
        ...
        :param torch.Tensor cn_memory: encoded cn memory (multi-enc), float32  (batch, maxlen_in, feat)
        :param torch.Tensor en_memory: encoded en memory (multi-enc), float32  (batch, maxlen_in, feat)
        ...
        """
        x = self.embed(tgt)
        x, tgt_mask, cn_memory, en_memory, memory_mask = self.decoders(x, 
                                            tgt_mask, cn_memory, en_memory, memory_mask)
        if self.normalize_before:
            x = self.after_norm(x)
