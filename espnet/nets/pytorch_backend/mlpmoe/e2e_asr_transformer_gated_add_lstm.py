...
class EncoderAggregrationLSTM(torch.nn.Module):
    def __init__(self, idim, odim, num_layers=1, hidden_dim=256, 
                bi=True, drop=0.1):
        super(EncoderAggregrationLSTM, self).__init__()
        # Note: dropout is not activated when lstm num_layers=1
        self.lstm = torch.nn.LSTM(idim, hidden_dim, num_layers=num_layers, 
                        bidirectional=bi, batch_first=True, dropout=drop)
        proj_hidden = 2*hidden_dim if bi else hidden_dim
        self.proj_module = torch.nn.Sequential(
                torch.nn.Dropout(p=drop, inplace=False),
                torch.nn.Linear(proj_hidden, odim),
                torch.nn.Sigmoid())

    def forward(self, xs, length):
        xs = pack_padded_sequence(xs, length, batch_first=True)
        xs, _ = self.lstm(xs)
        xs, _ = pad_packed_sequence(xs, batch_first=True)
        xs = self.proj_module(xs)
        return xs
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        self.aggregation_module = EncoderAggregrationLSTM(
            idim=2*args.adim, 
            odim=lambda_dim, 
            num_layers=1, 
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        # transform ilens here(conv2d embedding)
        ilens = (((ilens - 1) // 2 - 1) // 2)
        lambda_ = self.aggregation_module(hs_pad, ilens) # (B,T,1)/(B,T,D), range from (0, 1)
    ...
    def encode(self, x):
        ...
        """
        self.eval()
        ilens = torch.from_numpy(numpy.asarray([x.shape[0]]))
    ...
    def store_penultimate_state(self, xs_pad, ilens, ys_pad):
        ...
        ilens = (((ilens - 1) // 2 - 1) // 2)
        lambda_ = self.aggregation_module(hs_pad, ilens) # (B,T,1), range from (0, 1)
        hs_pad = lambda_ * cn_hs_pad + (1 - lambda_) * en_hs_pad
