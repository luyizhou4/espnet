...
from espnet.nets.pytorch_backend.transformer.encoder_aux_ali import Encoder
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        ...
        group.add_argument('--ali-file', default=None, type=str, required=True,
                           help='aux ali file path')
        group.add_argument('--ali-sampled', default=False, type=strtobool,
                           help='aux ali sampled')
        group.add_argument('--aux-pos', default='COVOUT', type=str,
		                   help='position where auxiliry bottleneck concatenate: COVOUT, ENOUT')
        group.add_argument('--aux-only', default=False, type=strtobool,
    ...
    def forward(self, xs_pad, ilens, ys_pad, uttid_list, train):
        """E2E forward.
        ...
        hs_pad, hs_mask = self.encoder(xs_pad, src_mask, uttid_list, train)
        self.hs_pad = hs_pad
    ...
    def encode(self, x):
        ...
        enc_output, _ = self.encoder(x[0], None, x[1], x[2])
    ...
    def calculate_all_attentions(self, xs_pad, ilens, ys_pad, uttid_list, train):
        """E2E attention calculation.

        :param torch.Tensor xs_pad: batch of padded input sequences (B, Tmax, idim)
        :param torch.Tensor ilens: batch of lengths of input sequences (B)
        :param torch.Tensor ys_pad: batch of padded token id sequence tensor (B, Lmax)
        :param list uttid_list: batch of uttid list (B)
        ...
        """
        with torch.no_grad():
            self.forward(xs_pad, ilens, ys_pad, uttid_list, train)
        ret = dict()
