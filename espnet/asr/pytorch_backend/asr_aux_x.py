...
from espnet.utils.io_utils_x import LoadInputsAndTargets
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device('cpu')):
        ...
        # batch should be located in list
        assert len(batch) == 1
        xs, ys, uttid_list, train = batch[0]
        ...
        return xs_pad, ilens, ys_pad, uttid_list, train
...
def train(args):
    ...
    if args.init_model is not None:
        logging.warning('init model with {}'.format(args.init_model))
        init_model, _ = load_trained_model(args.init_model)
    ...
    if args.moe_mode:
        ...
        converter = CustomConverterMoE(subsampling_factor=module.subsample[0], dtype=dtype)
        logging.warning("Using CustomConverterMoE")
    else:
        if args.num_encs == 1:
            converter = CustomConverter(subsampling_factor=module.subsample[0], dtype=dtype)
        else:
            converter = CustomConverterMulEnc([i[0] for i in module.subsample_list], dtype=dtype)
    ...
    load_tr = LoadInputsAndTargets(
        mode='asr', load_output=True, preprocess_conf=args.preprocess_conf, # if args.do_preprocess else None,
            ...
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.store_penultimate_state:
                    ...
                    if train_args.moe_mode:
                        ...
                        feat = [feat[0][0], feat[1], feat[2]] if args.num_encs == 1 else [feat[idx][0] for idx in range(model.num_encs)]
